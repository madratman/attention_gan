...
if FLAG_TRAIN_TEST_SPLIT_FROM_SCRATCH==1:
	...
	for idx in range(len(training_image_paths)):
		category_id = training_image_paths[idx].split('.')[0] 
		image_basename = training_image_paths[idx].split('.')[1].split('/')[1] # image_basename contains category_name 
		extension = training_image_paths[idx].split('.')[2]
		reqd_filename = '_'.join([category_id, '.'.join([image_basename, extension])]) 
		
		new_filename = os.path.join('data/train', reqd_filename)
	...
	for idx in range(len(testing_image_paths)):
		category_id = testing_image_paths[idx].split('.')[0] 
		image_basename = testing_image_paths[idx].split('.')[1].split('/')[1] # image_basename contains category_name 
		extension = testing_image_paths[idx].split('.')[2]
		reqd_filename = '_'.join([category_id, '.'.join([image_basename, extension])]) 
		
		new_filename = os.path.join('data/test', reqd_filename)
